    ...
    <OperationsShell>
      ...
      <div className="lims-card overflow-x-auto">
        <table className="lims-table">
          ...
          <tbody>
            {equipment.map((eq) => {
              const dueSoon = eq.nextCalibrationDue && new Date(eq.nextCalibrationDue) < new Date(Date.now() + 30 * 86400000);
              return (
                <tr key={eq.id}>
                  ...
                  <td>
                    {dueSoon && eq.status === 'Active' ? (
                      ...
                      <StatusBadge label={eq.status} variant={eq.status === 'Active' ? 'success' : 'warning'} />
                    )}
                  </td>
                </tr>
              );
