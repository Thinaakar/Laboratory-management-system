...
export const SESSION_COOKIE = 'labcore_session';
...
function secret(): string {
  return process.env.SESSION_SECRET ?? 'dev-labcore-session-secret';
}
...
export function parseSessionToken(token: string | undefined | null): SessionPayload | null {
  ...
  try {
    ...
    if (!payload.email || !payload.userId || !payload.role) return null;
    return payload;
  } catch {
    return null;
  }
